...
function ApplySelectedStyle(container) {
  ...
  StickerStructureContainer.innerHTML = `
    <div class="sticker-star-container"><img draggable="false" class="star-img" src="assets/stickers/Collections_Star_${StickerRarity}Star.png"></div>
    <div class="sticker-photo-container"><img draggable="false" class="sticker-card" src="stickers/${StickerSetPath}/${ImageSource}">${FrameHTML}</div>
    <div class="sticker-ribbon" style="background: ${RibbonEdgeColour}; background: -moz-linear-gradient(90deg, ${RibbonEdgeColour} 0%, ${Colour} 10%, ${Colour} 90%, ${RibbonEdgeColour} 100%); background: -webkit-linear-gradient(90deg, ${RibbonEdgeColour} 0%, ${Colour} 10%, ${Colour} 90%, ${RibbonEdgeColour} 100%); background: linear-gradient(90deg, ${RibbonEdgeColour} 0%, ${Colour} 10%, ${Colour} 90%, ${RibbonEdgeColour} 100%); border: 2px solid ${DarkenedColour};"><span class="${StickerNameClass}"><span><span data-translation-key="set">${SetText}</span>&nbsp;${StickerSet}&nbsp;&nbsp;#${StickerSetNo}</span><span class="StickerNameText" data-stickerid="${GlobalID}"></span></span>
    </div>
  `;
...
function PerformSort(event) {
  ...
  if (AscendZeroDescendOne === 1) {
    Array.from(document.querySelector("#sticker-board #current-sticker-board").children).reverse().forEach(child => { document.querySelector("#sticker-board #current-sticker-board").appendChild(child); });
  }
...
importBtn.addEventListener("click", () => {
  const importBtnText = importBtn.querySelector("#import-btn-text");
  ...
  setTimeout(function () {
    ...
    importBtnText.textContent = ImportSuccessfulText;
    setTimeout(function () {
      importBtnText.textContent = LANGUAGE_DICTIONARY.find(item => item["translation-key"] === "import")[CurrentLanguageCode];
    }, 3000);
  }, 0);
});
...
importFromFileBtn.addEventListener("click", () => {
  ...
  fileInput.addEventListener("change", () => {
    ...
    const importFromFileBtnText = importFromFileBtn.querySelector("#import-from-file-btn-text");
    setTimeout(function () {
      const ImportSuccessfulText = LANGUAGE_DICTIONARY.find(item => item["translation-key"] === "ImportSuccessful")[CurrentLanguageCode];
      importFromFileBtnText.textContent = ImportSuccessfulText;
      setTimeout(function () {
        importFromFileBtnText.textContent = LANGUAGE_DICTIONARY.find(item => item["translation-key"] === "LoadFromText")[CurrentLanguageCode];
      }, 3000);
    }, 0);
  });
  fileInput.click();
...
exportFromFileBtn.addEventListener("click", () => {
  ...
  link.download = `mogotools-userData_${year}${month}${day}_${hours}${minutes}${seconds}.txt`;

  link.href = url;
  link.click();

  const exportFromFileBtnText = exportFromFileBtn.querySelector("#export-from-file-btn-text");
  setTimeout(function () {
      ...
      .then(function (canvas) {
        ...
        if (navigator.userAgent.indexOf("Safari") > -1) {
          ...
          canvas.toBlob(function (blob) {
            ...
            link.download = `mogotools-screenshot_${year}${month}${day}_${hours}${minutes}${seconds}.png`;
            link.click();
          });
        } else {
...
if (dlPngButton) {
  ...
  dlPngButton.addEventListener("click", function () {
    ...
    const dlPngButtonText = dlPngButton.querySelector("#dl-png-btn-text");
    dlPngButtonText.textContent = DownloadingText;
    dlPngButton.classList.add("btnYellow");

    if(ImgOrientationLandscapeZeroPortraitOne === 1){      
      document.getElementById("snapshot-area").style.width = "400px";
    } else{
...
function GenerateTradePostClipboard() {
  ...
  const addTradePostLine = (userDataEntry, tradePostLines, isGoldenSymbol) => {
  ...
  for (const key in userData) {
    if (userData[key].lookingfor === 1 && userData[key].show === 1) {
      addTradePostLine(userData[key], tradePostLinesLF, "(G)");
    }
    if (userData[key].fortrade === 1 && userData[key].show === 1) {
      addTradePostLine(userData[key], tradePostLinesFT, "(G)");
    }
  }
...
function copyTradePostAreaToClipboard() {
  ...
  navigator.clipboard.writeText(tradePostText)
    .then(() => {
      ...
      const copyButtonText = document.querySelector("#copy-trade-post-area-btn-text");
      ...
      copyButtonText.textContent = CopiedText;
      setTimeout(() => {
        ...
        copyButtonText.textContent = CopyText;
      }, 3000);
    })
    .catch((err) => {
